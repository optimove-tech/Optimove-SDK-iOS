...
class OptimoveNotificationHandler {
    ...
    private func handleSdkCommand(command: OptimoveSdkCommand,
        ...
        case .ping:
            ...
            OptiLoggerMessages.logRemainBackgroundTime(backgroundTimeRemaining: UIApplication.shared.backgroundTimeRemaining)
    ...
    private func handleDeepLinkDelegation(_ response: UNNotificationResponse) {
        guard let dynamicLink = response.notification.request.content.userInfo[OptimoveKeys.Notification.dynamikLink.rawValue] as? String,
        ...
        Optimove.shared.deepLinkComponents = OptimoveDeepLinkComponents(screenName: String(urlComp.path.dropFirst()), parameters: params) // The dropFirst() is to eliminate the "/" prefix of the path
    }
