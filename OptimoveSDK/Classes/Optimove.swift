...
extension Optimove {
    ...
    @objc public func setUserId(_ userID: String) {
        ...
        let function: (ServiceLocator) -> Void = { serviceLocator in
            ...
            syncronizer.handle(.migrateUser)
        }
        container.resolve(function)
    }
...
extension Optimove {
    ...
    @objc public func startTestMode() {
        let function: (ServiceLocator) -> Void = { serviceLocator in
            tryCatch {
                ...
                serviceLocator.synchronizer().handle(.subscribeToTopic(topic: testTopic))
            }
        }
        container.resolve(function)
    }
    ...
    @objc public func stopTestMode() {
        let function: (ServiceLocator) -> Void = { serviceLocator in
            tryCatch {
                let testTopic = OptimoveKeys.testTopicPrefix + (try Bundle.getApplicationNameSpace())
                serviceLocator.synchronizer().handle(.unsubscribeFromTopic(topic: testTopic))
            }
        }
        container.resolve(function)
    }
