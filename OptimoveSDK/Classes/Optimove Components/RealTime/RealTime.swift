...
class RealTime: OptimoveComponent {
    ...
    func reportScreenEvent(customURL: String, pageTitle: String, category: String?) {
        let event = OptimoveEventDecorator(
            event: PageVisitEvent(customURL: customURL, pageTitle: pageTitle, category: category)
        )
    ...
    func report(eventDecor event: OptimoveEventDecorator, withConfigs config: OptimoveEventConfig) {
        ...
        if OptimoveUserDefaults.shared.realtimeSetUserIdFailed {
            ...
            if let config = Optimove.shared.eventWarehouse?.getConfig(ofEvent: setUserId) {
                let eventDecorator = OptimoveEventDecoratorFactory.getEventDecorator(
                    forEvent: setUserId,
                    withConfig: config
                )
        ...
        realTimeQueue.async {
            let rtEvent = RealtimeEvent(
                tid: self.metaData.realtimeToken,
                cid: OptimoveUserDefaults.shared.customerID,
                visitorId: VisitorID,
                eid: String(config.id),
            ...
            self.deviceStateMonitor.getStatus(of: .internet) { (online) in
                ...
                do {
                    ...
                    NetworkManager.post(
                        toUrl: URL(string: Optimove.shared.realTime.metaData.realtimeGateway + "reportEvent")!,
                        json: data
                    ) { (response, error) in
                ...
                do {
                    ...
                    NetworkManager.post(
                        ...
                        if error != nil {
                            ...
                            OptiLoggerMessages.logRealtimeSetUserIdStatus(
